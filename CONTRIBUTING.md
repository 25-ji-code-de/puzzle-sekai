...
```bash
# Fork 仓库后克隆到本地
git clone https://github.com/your-username/puzzle-sekai.git
cd puzzle-sekai

# 添加上游仓库
git remote add upstream https://github.com/25-ji-code-de/puzzle-sekai.git
```
...
- hooks 通过 `yarn install` 后可用的依赖进行驱动；如果本地 hook 丢失或 hook 配置更新后未生效，可手动执行 `yarn hooks:install`
...
- 纯工具函数优先放 `src/util/`（clamp / color / pad / format / hash / date-key / css-class / dialog-class / dev-log / nearest / manhattan / minmax / ease / json / number）
...
- 浏览现有的 [Issues](https://github.com/25-ji-code-de/puzzle-sekai/issues)
- 在 Issue 中提问
- 查看其他 SEKAI 生态项目的实现
