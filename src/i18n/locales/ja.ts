export const ja = {
  ...
  settings: {
    ...
    display: {
      ...
      windowedWebHelp: "通常のページ表示。開始時に自動で全画面にはなりません。",
      borderlessHelp:
        "枠なしウィンドウを最大化して画面いっぱいに表示（デスクトップ版のみ）。",
      fullscreenHelp: "フルスクリーン（開始時または切替時に入ります）。",
    },
    replays: {
      label: "リプレイ",
      watch: "リプレイを見る",
  ...
  controls: {
    ...
    swipeMove: "押し込みで仮想スティック · 位置が基礎落下に速度を加算",
    tapRotate: "左右半分を短くタップで回転（左=反時計 / 右=時計）",
    swipeHardDrop:
      "中心長押しでハードドロップ蓄力 · 一度外に出して戻ると遅い蓄力（約2.4秒）",
    pressSoftDrop: "スティック下方向でソフトドロップ（横移動と併用可）",
    easterEgg:
