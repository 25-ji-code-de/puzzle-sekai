 ...
 *
 * 2√ó2 pieces (NeneRobo / Mikudayo) tip the same way as horizontal 2-cell pieces:
 * only one bottom column supported ‚Ü?rotate 90¬∞ over the support edge.
 */
...
/**
 * Roots must span ‚â? columns so there is a hang side vs support side.
 * Covers horizontal 2-cell pieces AND 2√ó2 NeneRobo/Mikudayo.
 */
 ...
 *
 * tip right (dir=+1), pivot P ‚Ä?CW around bottom-right of P:
 *   support P      ‚Ü?hang column, same row
 *   hang H         ‚Ü?one row below hang column
 *   cell above P   ‚Ü?pryed up & over to the right
 *   2√ó2 top-right  ‚Ü?swings down-right with the hang
 *
 * tip left mirrored.
 */
const rotateCell = (
...
/** Rotate a pixel point around fulcrum by angle Œ∏ (y-down, +Œ∏ = CW). */
const rotatePoint = (
  x: number,
  y: number,
  fx: number,
...
const findTipPlan = (list: SpriteData[]): TipPlan | null => {
  ...
  // Lowest first ‚Ä?foundations tip before free-floating tops
  const order = list
    .map((sp, index) => ({
      index,
      maxY: maxFootprintY((sp.coordinates ?? []) as Cell[]),
      // Prefer wider / bigger roots first (2√ó2 before a 2-cell sitting on it)
...
/** Write footprint directly ‚Ä?never re-derive cells from sprite math after a tip. */
const commitTipLanding = (sp: SpriteData, newCells: Cell[]) => {
  ...
  if (kind === "cell2") {
    // Match piece.ts convention: spawn at œÄ, orient 0 ‚Ü?œÄ, orient 1 ‚Ü?œÄ + œÄ/2, ‚Ä?
    // getOffset: (rotation/œÄ * 2 + 2) % 4 ‚Ä?we only need getOffset === orient.
 ...
 * Pieces on the support side arc upward (ÁøòËµ∑Êù?, hang side swings down.
 * 2√ó2 squares rotate as a rigid body the same way.
 */
    ...
    const tick = (delta: number) => {
      ...
      if (t >= 1) {
        ...
        members.forEach(({ sp }, i) => {
          ...
          // Commit cells by tip geometry ‚Ä?never via getCoordinates
